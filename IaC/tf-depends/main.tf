provider "aws" {
  region = "ap-south-1"
  profile = "fahad"
}

resource "aws_instance" "ec2_example" {
  ami           = "ami-08188a5a4dfdbd573"
  instance_type = "t3.micro"
  
  vpc_security_group_ids = [aws_security_group.main.id]
  key_name = aws_key_pair.deployer.key_name

  user_data = "${file("install_apache.sh")}"

  tags = {
    Name  = "tf-states"
    Owner = "fahad"
    Event = "learning tf"
  }
  depends_on = [ aws_s3_bucket.s3_bucket_created, aws_security_group.main, aws_key_pair.deployer ]
}

resource "aws_s3_bucket" "s3_bucket_created" {
  bucket = "fmk-tf-s3-bucket"

  tags = {
    Name  = "fmk-tf-s3-bucket"
    Owner = "fahad"
    Event = "learning tf"
  }

}


resource "aws_security_group" "main" {
  egress = [
    {
      cidr_blocks      = ["0.0.0.0/0"]
      description      = ""
      from_port        = 0
      ipv6_cidr_blocks = []
      prefix_list_ids  = []
      protocol         = "-1"
      security_groups  = []
      self             = false
      to_port          = 0
    }
  ]
  ingress = [
    {
      cidr_blocks      = ["0.0.0.0/0"]
      description      = ""
      from_port        = 22
      ipv6_cidr_blocks = []
      prefix_list_ids  = []
      protocol         = "tcp"
      security_groups  = []
      self             = false
      to_port          = 22
    }
  ]

}

resource "aws_key_pair" "deployer" {
  key_name   = "aws_key"
  public_key = "ssh-rsa  bjit@11713-fahad.kamal"
}